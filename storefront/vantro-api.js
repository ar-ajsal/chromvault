document.addEventListener("DOMContentLoaded", async () => {
    ...
    function getTitle(product) {
        ...
        if (product.title && typeof product.title === "object") return product.title.en || "Product";
    ...
    if (window.location.pathname.startsWith("/products/")) {
        ...
        try {
            ...
            // Expose product data to the early-capture intercept in <head>
            window.__vantro_product__ = { id: product._id || product.id, title: getTitle(product), price: getPrice(product), image: getImage(product) };

            // If cart contains temporary entry with id == slug, update it to real id
            ...
            // Title: only update heading elements
            for (const sel of ["h1.product__title", ".product__title h1", "h1"]) {
                const els = document.querySelectorAll(sel);
                if (els.length) { els.forEach(el => { el.innerText = title; }); break; }
            }

            // Price: prefer specific selectors; fallback to leaf nodes
            ...
            // Images
            if (imgUrl) { document.querySelectorAll("img").forEach(img => { const s = img.src || ""; if (s.includes("cdn.shopify.com") || s.includes("vantro.com") || s.includes("/cdn/shop/")) { img.src = imgUrl; img.srcset = ""; } }); }
            ...
            // Add to Cart: intercept form submit in CAPTURE phase (runs before Shopify listener)
    ...
    if (window.location.pathname.startsWith("/checkout")) {
        ...
        // Build checkout UI if the page does not already have the form (checkout.html is a re-used cart template)
        if (!document.getElementById("checkout-form")) {
            const mainEl = document.querySelector("main, #MainContent, .main-content, .page-content, body");
            const existingContent = mainEl ? mainEl.innerHTML : "";
            const checkoutHtml = `
        ...
        // --- Fetch and populate Indian states and districts ---
    ...
    // --- HERO MEDIA ---
    const heroMedia = document.querySelector(".hero-bottom-media");
    if (heroMedia) {
        try {
            const res = await fetch(`${apiBase}/settings/hero_media`);
            if (res.ok) { const d = await res.json(); if (d.value) { const u=d.value,isV=/\.(mp4|webm)$/.test(u)||u.includes("/video/upload/"); heroMedia.outerHTML=isV?`<video class="hero-bottom-media" src="${u}" autoplay loop muted playsinline style="width:100%;height:100%;object-fit:cover;"></video>`:`<img class="hero-bottom-media" src="${u}" style="width:100%;height:100%;object-fit:cover;"/>`; } }
        } catch(e){}
    }
            ...
            if (cats.length) {
                document.querySelectorAll(".mobile-menu-drawer__collections-list, [class*=collections-list]").forEach(c => {
                    ...
                    cats.forEach(cat => {
                        ...
                        a.href=`/collections/${slug}`;a.className="mobile-menu-drawer__collection-link";a.innerText=name.toUpperCase();
                        c.appendChild(a);
                    });
                });
    ...
    // Unhide the page once hydration finishes
