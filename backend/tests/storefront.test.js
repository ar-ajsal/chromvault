 ...
 *   3. That the retired WordPress surface and the old /chromora.in/ prefix are
...
(async () => {
  ...
  check('shell injects API base', home.body.includes('window.__CHROMORA_API_BASE__="/v1"'), true);
  check('shell injects Maps key when configured',
    home.body.includes('window.__CHROMORA_MAPS_KEY__="test-maps-key-not-a-real-key"'), true);
  check('injection precedes config.js',
    home.body.indexOf('__CHROMORA_API_BASE__') < home.body.indexOf('assets/js/config.js'), true);
  ...
  check('service worker cache names are storefront-scoped',
    sw.body.includes("'chromora-store-'"), true);
  ...
  /* ── Canonical URLs ────────────────────────────────────────────────────── */
  const legacyPrefix = await get('/chromora.in/shop');
  check('/chromora.in/shop -> 301', legacyPrefix.status, 301);
  check('/chromora.in/shop -> /shop', legacyPrefix.headers.location, '/shop');

  const legacyQuery = await get('/chromora.in/shop?category=rings');
  check('/chromora.in/… keeps the query', legacyQuery.headers.location, '/shop?category=rings');
